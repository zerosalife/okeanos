...
- [-] Enemy death animation via a delegate function that controls
  ...
  - [ ] Add shader to Boss, as well.
  - [ ] Freeze the game for 20 ms so there's some weight to the
...
- [ ] More bass in the hit sounds.
- [ ] Pop up with exp gained when enemies are killed
